...
if auto_preview:
    with st.sidebar.expander("Live preview", expanded=True):
        # generate a smaller preview dataset for speed
        preview_n = min(300, max(20, int(n_points)))
        Xp, yp = generate_data(a=a, b=b, n=preview_n, noise=noise, seed=int(seed))
        # fit a quick model to show fitted line
        try:
            res_preview = fit_linear_regression(Xp, yp, fit_intercept=fit_intercept)
            coef_p = res_preview['coef']
            intercept_p = res_preview['intercept']
        except Exception:
            coef_p = a
            intercept_p = b

        figp, axp = plt.subplots(figsize=(4, 3))
        axp.scatter(Xp.ravel(), yp.ravel(), alpha=0.6, s=10)
        xs = np.linspace(Xp.min(), Xp.max(), 200)
        axp.plot(xs, a * xs + b, color='green', label='true line')
        axp.plot(xs, coef_p * xs + intercept_p, color='red', linestyle='--', label='fitted line')
        axp.legend(fontsize='small')
        axp.set_xlabel('x')
        axp.set_ylabel('y')
        axp.set_title('Live preview')
